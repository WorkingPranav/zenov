...
async def forcesub(c: Client, m: Message):
    ...
    if not_joined_channels := [ch for ch in channel_status if not ch["joined"]]:
        ...
        markup.append(
            [InlineKeyboardButton(text="↺ Refresh", callback_data=f"refresh_{command}")]
        )
...
async def refresh_cb(c: Client, m):
    ...
    if not_joined_channels := [ch for ch in channel_status if not ch["joined"]]:
        ...
        await m.message.edit(
            ...
            f"𝖧𝖾𝗒 {mention}, 𝖯𝗅𝖾𝖺𝗌𝖾 𝖩𝗈𝗂𝗇 𝗆𝗒 𝖢𝗁𝖺𝗇𝗇𝖾𝗅𝗌 𝖺𝗇𝖽 𝗍𝗁𝖾𝗇 𝖼𝗅𝗂𝖼𝗄 𝗈𝗇 '↺𝖱𝖾𝖿𝗋𝖾𝗌𝗁' 𝗍𝗈 𝗀𝖾𝗍 𝗒𝗈𝗎𝗋 𝖤𝗉𝗂𝗌𝗈𝖽𝖾(𝗌) ⤋",
            reply_markup=InlineKeyboardMarkup(markup),
        )
        return
